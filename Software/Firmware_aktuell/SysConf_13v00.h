#pragma once
// SysConf_13v00.h – Konfigurationskonstanten für bTn Wecker
// Firmware-Version : 13v00
// Datei-Version    : 13v00
// Boardverwalter   : esp32 3.3.11 von Espressif Systems
//
// Änderungshistorie:
//   13v00–Root Cause von "kein Start-Status nach playFolder" (st=-1) gefunden:
//         DFRobotDFPlayerMini::readState() gibt bei JEDER Nicht-Feedback-
//         Frame-Art (z.B. unaufgeforderter Play-Finished-Meldung 0x3C/0x3D
//         vom vorherigen Titel) sofort -1 zurück, ohne weiter auf die
//         tatsächliche Antwort zu warten. Der zuvor vermutete Zusammenhang
//         mit dem ACK-Modus (0x41-Frame) traf NICHT zu – parseStack() der
//         Bibliothek verschluckt 0x41 bereits intern, unabhängig vom Fund.
//   13v00–Fix 1: readStateDrained() wartet nach einem -1 von readState() noch
...
//         Bedienvorgängen blieben sonst liegen, bis ein zufälliger späterer
//         readStateDrained()-Aufruf sie mit abräumte – im ungünstigen Fall
//         genau während der Alarm-Verifikation noch vorhanden.
//   12v18–12v17-Fix griff nicht während ALARM_RUNNING: checkSerial2Leftover()
//         setzte serial2LeftoverLastLogged bei avail==0 zurück auf 0 – bei
//         jedem Poll-Zyklus wird der Puffer aber zwischen den beiden
//         readState-Abfragen durch readStateDrained() leergezogen, sodass
//         derselbe wiederkehrende Restbyte-Wert (Poll → Restbytes → auf 0
//         gedraint → nächster Poll dieselbe Anzahl) jedes Mal erneut als
//         "neue" Änderung gewertet und geloggt wurde. Fix: der zuletzt
//         gemeldete Wert bleibt jetzt über Nullstände hinweg erhalten
//         (Startwert -1 statt 0) – nur ein tatsächlich anderer Restbyte-Wert
//         löst noch eine neue Web-Log-Zeile aus.
...
// ── Firmware-Version ─────────────────────────────────────────
#define FW_VERSION "13v00"                                                     // Versionsnummer (als String in PGMInfo, Web-Log, WEB.h)
