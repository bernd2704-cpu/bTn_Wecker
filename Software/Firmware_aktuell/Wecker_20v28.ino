...
// ── Konfiguration ────────────────────────────────────────────
#include "SysConf_20v28.h"                                                               // Pin-Belegung, Timing-Konstanten, Touch-Schwellwerte
#include "WEB.h"
...
// 20v12 (C6-Fix, Audit 2026-08-13): von S1/Sound-Vorschau/Funktionswahl
// gesetzt, wenn player.stop() einen laufenden Alarmversuch trifft (siehe
// requestAlarmCancelIfActive()). verifyPlayStarted() wertet es als
// regulären Nutzerabbruch statt als Absturz – ohne dieses Signal sah
// triggerAlarm() den absichtlich gestoppten Player als Fehlschlag und
// startete entweder neu (ESP.restart(), löste denselben Alarm nach dem
// Reboot erneut aus) oder aktivierte alarmSilentFallback (Motor/Licht
// wieder an, obwohl der Nutzer gerade gestoppt hat).
volatile bool alarmCancelRequested = false;
...
void checkboxSound() {
  ...
  display.setColor(WHITE);
  switch (pageselect) {
    // 20v12 (C7-Fix, Audit 2026-08-13): sound*_assigned erst nach erfolgreichem
    // Mutex-Take übernehmen – vorher wurde die Zuordnung unbedingt VOR dem Take
    // gesetzt; schlug der Take fehl, zeigte die Checkbox einen Sound an, den der
    // Player nie bekommen hatte. (C6-Fix): vor player.stop() zusätzlich
    // requestAlarmCancelIfActive() – die Vorschau könnte sonst einen zeitgleich
    // laufenden Alarmversuch als Absturz erscheinen lassen (siehe dort).
    case 3:
      if (sound1_on) {
        ...
        if (xSemaphoreTake(playerMutex, pdMS_TO_TICKS(50)) == pdTRUE) {                  // 50 ms < 100 ms displayMutex-Timeout
          ...
          webLogf("[FEHLER] Sound1-Vorschau fehlgeschlagen (Mutex belegt)");
        }
      } else {
        display.drawRect(67, 37, 10, 10);
    ...
    case 4:
      if (sound2_on) {
        ...
        if (xSemaphoreTake(playerMutex, pdMS_TO_TICKS(50)) == pdTRUE) {                  // 50 ms < 100 ms displayMutex-Timeout
          ...
          webLogf("[FEHLER] Sound2-Vorschau fehlgeschlagen (Mutex belegt)");
        }
      } else {
        display.drawRect(67, 54, 10, 10);
        display.display();                                                               // Checkbox anzeigen bevor Player ggf. gestoppt
...
void writeNVR() {
  ...
  data.putInt ("vol",         vol);
  data.putInt ("lastA1Day",   lastA1Day);                // 20v17: Alarm-Tages-Sperre, siehe lockAlarmDayGuard()
  data.putInt ("lastA2Day",   lastA2Day);
}
...
void readNVR() {
  ...
  // 20v17: Alarm-Tages-Sperre (siehe lockAlarmDayGuard()) – ersetzt den
  // RTC_NOINIT-Ansatz aus 20v16, der Reset-Taster/Neuflashen nicht überstand.
  lastA1Day   = data.getInt ("lastA1Day",    lastA1Day);
  lastA2Day   = data.getInt ("lastA2Day",    lastA2Day);
  ...
  if (lastA1Day != 0xFFFF && lastA1Day > 365) lastA1Day = 0xFFFF;  // yday 0–365, sonst ungültig
  if (lastA2Day != 0xFFFF && lastA2Day > 365) lastA2Day = 0xFFFF;
}
...
// 20v22 (Bugfix): Tages-Sperre (lastA1Day/lastA2Day) wird erst HIER beim
// Verlassen der jeweiligen Alarm-Seite aufgehoben, nicht mehr bei jedem
// einzelnen Stunde+/Minute+-Tastendruck in onAlarm1()/onAlarm2() – dort lag
// die neue Weckzeit während des Durchscrollens (z.B. 10:00 → 14:30) kurzzeitig
// auf Zwischenwerten, von denen manche im 500-ms-Takt des Haupt-Loops sofort
// als "fällig" gegen alarmDue() erkannt wurden und den Alarm auslösten
// (gemeldet 25.08.2026). uiTransition() ist der einzige Ort, über den JEDER
...
static UiState onClock(uint8_t evt) {
  switch (evt) {
    // 20v12 (C7-Fix, Audit 2026-08-13): vol/Anzeige/NVS erst nach erfolgreichem
    // Mutex-Take ändern – vorher lief vol++/-- unbedingt VOR dem Take, Anzeige
    // und markSafeChange() liefen ebenfalls unbedingt danach. Hielt alarmTask
    // den Mutex gerade (5-s-Poll bzw. verifyPlayStarted()), zeigte die Anzeige
    // einen Wert, den der Player nie bekommen hatte, und genau dieser falsche
    // Wert landete in NVS – die Lautstärkeregelung wirkte dann nicht, obwohl
    // sie das gerade sollte (Alarm läuft, Nutzer will leiser stellen).
    case EVT_T3:                                                                         // Lautstärke +
      if (vol < MAX_VOL) {
        if (xSemaphoreTake(playerMutex, pdMS_TO_TICKS(50)) == pdTRUE) {                 // 50 ms < 100 ms displayMutex-Timeout
          ...
          webLogf("[FEHLER] Lautstaerke+ fehlgeschlagen (Mutex belegt), vol unveraendert bei %u", vol);
        }
      }
      break;
...
static UiState onAlarm1(uint8_t evt) {
  switch (evt) {
    ...
    case EVT_T3:                                                                         // Stunde +
      ...
      // 20v22 (Bugfix): Tages-Sperre NICHT hier aufheben – a1_hour/a1_min werden
      // live vom Haupt-Loop gegen alarmDue() geprüft; während des Durchscrollens
...
// 20v16 (verworfen, siehe 20v17): Tages-Sperre zunächst über RTC_NOINIT_ATTR
// gespiegelt, in der Annahme, dieser Speicher überstehe auch Reset-Taster und
// Neuflashen. Das stimmt nicht: der EN-Pin (sowohl Reset-Taster als auch der
// harte Reset, den esptool/Arduino-IDE beim Neuflashen per DTR/RTS auslöst)
// schaltet den internen Spannungsregler des ESP32 kurz komplett ab – das ist
// elektrisch ein echter Power-On und löscht damit auch RTC_NOINIT-Speicher.
// Nur ESP.restart() (reiner Software-Reset, siehe rtcRetryMagic) lässt die
// RTC-Domäne durchgehend mit Strom versorgt. Der Fix griff dadurch bei
// Reset-Taster/Neuflashen nicht (Bug weiterhin reproduzierbar, gemeldet
// 2026-08-19).
//
// 20v17 (Bugfix): Tages-Sperre stattdessen im Flash-NVR gespiegelt (siehe
// readNVR()/writeNVR() sowie lockAlarmDayGuard()) – das übersteht zuverlässig
// jeden Neustart-Typ (Reset-Taster, Neuflashen, echter Stromausfall). Da der
// Wert erst NACH erfolgreichem Alarmstart geschrieben wird, ist das korrekt:
// ein bereits erfolgreich gespielter Alarm darf nach JEDEM Neustart-Typ nicht
// erneut auslösen, auch nicht nach einem echten Stromausfall kurz danach.

// 20v21 (Auftrag 21.08.2026): Der Wecker dient dazu, einen nach der Weckzeit
// liegenden Termin einzuhalten – der reale zeitliche Abstand Weckzeit→Termin
// muss daher unabhängig von der Zeitumstellung erhalten bleiben.
// dstSpringForwardToday: heute springt die Uhr von 01:59:59 auf 03:00:00
// CEST (letzter Sonntag im März) – die Wanduhrzeit 02:00:00–02:59:59
// existiert an diesem Tag gar nicht.
// dstFallBackToday: heute wird die Uhr von 02:59:59 CEST auf 02:00:00 CET
// zurückgestellt (letzter Sonntag im Oktober) – die Wanduhrzeit
// 02:00:00–02:59:59 kommt an diesem Tag zweimal vor (erst CEST, dann CET).
// Ermittlung TZ-generisch über mktime()/tm_isdst statt fest verdrahteter
// Datumsregel: Stunde 1 (vor jeder möglichen Umstellung) und Stunde 4 (nach
// jeder möglichen Umstellung) sind an beiden Umstellungstagen eindeutige,
// nie übersprungene/doppelte Wanduhrzeiten – ändert sich tm_isdst zwischen
// beiden, hat heute eine Umstellung stattgefunden (Richtung an isdst ablesbar).
static uint16_t dstFlagsYday          = 0xFFFF;
static bool     dstSpringForwardToday = false;
static bool     dstFallBackToday      = false;

static void updateDstDayFlags(const struct tm& nowTm) {
  if (nowTm.tm_yday == dstFlagsYday) { return; }                                          // 1x pro Kalendertag genügt
  dstFlagsYday = (uint16_t)nowTm.tm_yday;

  struct tm probe = nowTm;
  probe.tm_hour = 1; probe.tm_min = 0; probe.tm_sec = 0; probe.tm_isdst = -1;
  time_t    tBefore = mktime(&probe);
  struct tm rBefore;
  localtime_r(&tBefore, &rBefore);

  probe = nowTm;
  probe.tm_hour = 4; probe.tm_min = 0; probe.tm_sec = 0; probe.tm_isdst = -1;
  time_t    tAfter = mktime(&probe);
  struct tm rAfter;
  localtime_r(&tAfter, &rAfter);

  dstSpringForwardToday = (rBefore.tm_isdst == 0 && rAfter.tm_isdst == 1);
  dstFallBackToday      = (rBefore.tm_isdst == 1 && rAfter.tm_isdst == 0);
}
...
// abgelaufen ist).
// 20v21: Weckzeiten mit h==2 (die einzige Stunde, die an Umstellungstagen
// fehlt bzw. doppelt vorkommt) werden an diesen Tagen speziell behandelt,
// siehe updateDstDayFlags() oben. isdstNow unterscheidet im Herbst den
// ersten (CEST) vom zweiten (CET) Durchlauf durch die doppelte Stunde.
static bool alarmDue(bool on, uint8_t h, uint8_t m,
  ...
  uint8_t effH = h;
  if (dstSpringForwardToday && h == 2) {
    effH = (uint8_t)(h - 1);                                                              // fehlende Stunde: 1h vorziehen, Termin-Abstand bleibt gewahrt
  }
  if (dstFallBackToday && h == 2 && isdstNow == 1) {
    return false;                                                                          // erster (CEST-)Durchlauf: noch nicht auslösen, erst der zweite (CET)
  }

  int16_t diff = (int16_t)(hour * 60 + min) - (int16_t)(effH * 60 + m);
  return (diff >= 0 && diff <= ALARM_CATCHUP_MIN);
}

// Setzt die Tages-Sperre für alarmNum auf "heute" – gemeinsam genutzt vom
// regulären Erfolgspfad und vom C6-Abbruchpfad in triggerAlarm(). Muss in
// beiden Fällen laufen: ohne sie würde alarmDue() den soeben beendeten
// Alarm im nächsten 500-ms-Tick sofort erneut auslösen.
// 20v17: markSafeChange() zusätzlich gesetzt – nvrTask persistiert lastA1Day/
// lastA2Day damit im Flash-NVR (siehe writeNVR()) und übersteht so auch
// Reset-Taster/Neuflashen/Stromausfall (RTC_NOINIT-Ansatz aus 20v16 verworfen).
static void lockAlarmDayGuard(uint8_t alarmNum) {
  time_t    now_trigger;
  struct tm tm_trigger;
  time(&now_trigger);
  localtime_r(&now_trigger, &tm_trigger);
  uint16_t today = (uint16_t)tm_trigger.tm_yday;
  if (alarmNum == 1) { lastA1Day = today; }
  else               { lastA2Day = today; }
  markSafeChange();
}
...
// danach Sollwert. 20v20: Rail vor dem MOSFET wird per MCP1700T-3302E/TO auf
// 3,3V geregelt – der Kickstart-Vollgasimpuls (Duty 255) bleibt dadurch
// innerhalb der Rail-Spannung, keine Überspannung am 3-V-Motor mehr möglich.
// Das vTaskDelay des Kickstarts ist unkritisch:
...
static PlayVerifyResult verifyPlayStarted(const char* label, uint8_t fileNo) {
  ...
  for (uint8_t attempt = 1; attempt <= VERIFY_PLAY_RETRIES; attempt++) {
    ...
    if (alarmCancelRequested) {
      // 20v12 (C6-Fix): sofortiger Ausstieg statt der übrigen Versuche –
      // ein bewusster Nutzerabbruch soll nicht auf das Ende der Retry-
      // Schleife warten müssen.
      alarmCancelRequested = false;
      webLogf("[DFPlayer] %s: Wiedergabe waehrend Verifikation manuell gestoppt (S1/Vorschau/Menue) – kein Neustart", label);
...
static void triggerAlarm(uint8_t alarmNum, uint8_t fileNo, uint8_t min, uint8_t failCount) {
  ...
  if (xSemaphoreTake(playerMutex, pdMS_TO_TICKS(200)) == pdTRUE) {
    ...
    // 20v12 (E1-Fix, Audit 2026-08-13): Lautstaerke vor jedem Alarm explizit
    // setzen, mit Untergrenze ALARM_MIN_VOL. `vol` kann zwischenzeitlich auf
    // 0 gefallen sein (z.B. klemmendes Touch-Pad zaehlt T4 unbegrenzt herunter,
    // siehe Zwangs-Reset in touchTask) – ohne diesen Befehl liefe der Alarm
    // mit der zuletzt gesendeten, moeglicherweise stummen Lautstaerke. `vol`
    // selbst bleibt unveraendert – kein Einfluss auf Anzeige/NVS.
    uint8_t alarmVol = (vol < ALARM_MIN_VOL) ? ALARM_MIN_VOL : vol;
    player.volume(alarmVol);
    ...
    if (playResult == PLAY_CANCELLED) {
      // 20v12 (C6-Fix): Nutzer hat waehrend der Verifikation bewusst gestoppt
      // (S1/Sound-Vorschau/Menue) – kein Neustart, kein Ersatzalarm. Tages-
      // Sperre trotzdem setzen, sonst wuerde alarmDue() im naechsten Tick
      // sofort erneut auslösen.
      lockAlarmDayGuard(alarmNum);
      rtcRetryMagic = 0;
      alarmTriggerInFlight = false;
      return;
    }
    ...
    lockAlarmDayGuard(alarmNum);                                                          // 20v04 (A1-Fix): Tages-Sperre erst nach erfolgreichem Start setzen
...
// ── Alarm-State-Machine ──────────────────────────────────────
// sec/min/hour/yday/isdst: atomarer Zeitschnappschuss aus alarmTask – keine Race Condition mit displayTask
static void runAlarmMachine(uint8_t sec, uint8_t min, uint8_t hour, uint16_t yday, int isdst) {
  switch (alarmState) {
    ...
    case ALARM_IDLE:
      ...
      // Alarm 1 prüfen
      if (alarmDue(a1_on, a1_hour, a1_min, hour, min, yday, lastA1Day, isdst)) {
        triggerAlarm(1, sound1_assigned, min, 0);
      }
      // Alarm 2 prüfen (else if → Alarm 1 hat Vorrang bei gleicher Zeit)
      else if (alarmDue(a2_on, a2_hour, a2_min, hour, min, yday, lastA2Day, isdst)) {
...
static void touchTask(void *pvParam) {
  ...
  static const char* const PAD_LABEL[4] = { "T0", "T2", "T3", "T4" };                    // 20v12 (E1-Fix): fuer die Zwangs-Reset-Meldung
  ...
  while (true) {
    ...
    // 20v12 (E1-Fix, Audit 2026-08-13): dauerhaft klemmendes Pad (Feuchtigkeit/
    // Kondensation, Kabeldefekt) hatte bisher keinen Ausstieg außer dem
    // Loslassen selbst – TS_PRESSED/TS_REPEAT liefen unbegrenzt weiter, alle
    // TOUCH_REPEAT_MS ein Event (z.B. Lautstärke über T4 bis auf 0 herunter-
    // gezählt, siehe onClock()). Erzwungener Reset nach TOUCH_MAX_HOLD_MS
    // inkl. Baseline-Neuerfassung, die sonst hart an TS_IDLE gekoppelt ist
    // und bei einem klemmenden Pad nie wieder liefe.
    if ((tsState == TS_PRESSED || tsState == TS_REPEAT) && now - pressStart >= TOUCH_MAX_HOLD_MS) {
      webLogf("[WARNUNG] Touch %s klemmt seit %lu ms – Zwangs-Reset, Baseline neu gemessen",
              PAD_LABEL[activeIdx], (unsigned long)(now - pressStart));
      for (int i = 0; i < 4; i++) { touch_pad_read(TOUCH_PADS[i], &baseline[i]); }
      updateSnapTouch(baseline);
      lastRecal = now;
      tsState   = TS_IDLE;
      activeIdx = -1;
...
static void inputTask(void *pvParam) {
  // 20v13 (Schritt 11, Audit 2026-08-13): Rückgabewert prüfen – schlägt die
  // Anmeldung fehl, feuert esp_task_wdt_reset() unten ins Leere ("task not
  // found"), ohne dass irgendetwas darauf hindeutet.
  if (esp_task_wdt_add(NULL) != ESP_OK) { webLog("[FEHLER] TWDT: esp_task_wdt_add() fehlgeschlagen fuer inputTask"); }
  uint8_t evt;
    ...
    if (evt == EVT_S1) {
      ...
      if (t_now - lastBtnMs[0] >= BTN_LOCKOUT_MS) {
        ...
        if (alarmState == ALARM_RUNNING || st > 0) {
          // 20v12 (C6-Fix, Audit 2026-08-13): S1 kann auch treffen, während
          // triggerAlarm() noch in verifyPlayStarted() wartet (alarmState
          // hier noch ALARM_IDLE, aber st>0 zeigt bereits echte Wiedergabe) –
          // ohne dieses Signal sah verifyPlayStarted() den durch S1
...
static void displayTask(void *pvParam) {
  if (esp_task_wdt_add(NULL) != ESP_OK) { webLog("[FEHLER] TWDT: esp_task_wdt_add() fehlgeschlagen fuer displayTask"); }  // 20v13, siehe inputTask
...
static void alarmTask(void *pvParam) {
  if (esp_task_wdt_add(NULL) != ESP_OK) { webLog("[FEHLER] TWDT: esp_task_wdt_add() fehlgeschlagen fuer alarmTask"); }  // 20v13, siehe inputTask
  while (true) {
    ...
    const int      a_isdst = tm_alarm.tm_isdst;                                        // 20v21: unterscheidet im Herbst 1./2. Durchlauf der doppelten Stunde
    updateDstDayFlags(tm_alarm);                                                       // 20v21: Zeitumstellungstag erkennen, siehe alarmDue()
    runAlarmMachine(a_sec, a_min, a_hour, a_yday, a_isdst);
...
static void webLogTask(void *pvParam) {
  ...
  logServer.on("/", HTTP_GET, [&logServer]() {
    ...
    // alten Heap-Block freigeben → Fragmentierung.
    // 20v12 (D2-Fix, Audit 2026-08-13): 8 kB deckte den Worst Case nicht –
    // festes Markup (~2,5 kB) + 40 Ringpufferzeilen à bis 127 Zeichen +
    // Span-Overhead + Snapshots ergeben real bis zu ~9,8 kB. Auf 12 kB
    // angehoben (Audit-Minimalvorschlag statt vollständigem Umbau auf
    // sendContent()-Streaming).
    String html;
    html.reserve(12288);
    html +=
  ...
  logServer.on("/log", HTTP_GET, [&logServer]() {
    // 20v12 (D2-Fix): reserviert bisher gar nicht – WEBLOG_LINES (40) ×
    // WEBLOG_LINE_LEN (128) + Zeilenumbrüche passt bequem in 6 kB.
    String out;
    out.reserve(6144);
    if (webLogMutex && xSemaphoreTake(webLogMutex, pdMS_TO_TICKS(200)) == pdTRUE) {
...
void setup() {
  ...
  // (lastA1Day/lastA2Day) wieder her – siehe lockAlarmDayGuard().
  ...
  if (wifiConnected) {
    ...
    // 20v13 (Schritt 11, Audit 2026-08-13): Erfolgsmeldung + snapNtpTime nur
    // noch bei tatsächlichem Sync setzen – bisher liefen beide unbedingt nach
    // der Schleife, auch nach einem Timeout-break. Wer nach einem ausgefallenen
    // Alarm ins Log sieht, hätte einen Uhrzeitfehler damit fälschlich
    // ausgeschlossen (diagnostisch teuer im Zusammenspiel mit A1/A5).
  ...
  snprintf(str_a1,      sizeof(str_a1),      "%02u:%02u", a1_hour, a1_min);
  snprintf(str_a2,      sizeof(str_a2),      "%02u:%02u", a2_hour, a2_min);
  sound1_selected = sound1_assigned;                                                     // Sound-Auswahl startet bei der aktuell zugewiesenen mp3, nicht mehr immer bei 1
  sound2_selected = sound2_assigned;
  snprintf(str_s1,      sizeof(str_s1),      "%03u",      sound1_selected);
  snprintf(str_s2,      sizeof(str_s2),      "%03u",      sound2_selected);
  ...
  const esp_task_wdt_config_t twdt_cfg = {
    .timeout_ms    = WDT_HARDWARE_MS,  // aus SysConf_20v28.h
    .idle_core_mask = 0,               // Idle-Tasks nicht überwachen
    .trigger_panic  = true,            // Backtrace + Reset bei Ablauf
  };
  // 20v13 (Schritt 11, Audit 2026-08-13): Rückgabewert auswerten statt
  // "[TWDT] Hardware Watchdog aktiv" blind zu behaupten – ein Fehlschlag hier
  // wäre exakt der 12v15-Fehler (TWDT nie wirklich initialisiert) und bliebe
  // ohne diese Prüfung symptomfrei im Log.
  esp_err_t twdtErr;
  if (esp_task_wdt_status(NULL) == ESP_ERR_INVALID_STATE) {
    twdtErr = esp_task_wdt_init(&twdt_cfg);                                              // TWDT existiert noch nicht – anlegen
  } else {
    twdtErr = esp_task_wdt_reconfigure(&twdt_cfg);                                       // TWDT existiert schon (Boot-Default) – nur umkonfigurieren
  }
  if (twdtErr == ESP_OK) {
    webLogf("[TWDT] Hardware Watchdog aktiv (%u ms)", (unsigned)WDT_HARDWARE_MS);
  } else {
    webLogf("[FEHLER] TWDT: Init/Reconfigure fehlgeschlagen (esp_err_t=%d) – Hardware-Watchdog evtl. inaktiv", (int)twdtErr);
  }
