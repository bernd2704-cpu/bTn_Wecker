...
// ── Konfiguration ────────────────────────────────────────────
#include "SysConf_20v16.h"                                                               // Pin-Belegung, Timing-Konstanten, Touch-Schwellwerte
#include "WEB.h"
...
// 20v16 (Bugfix): Tages-Sperre zusätzlich im RTC-Speicher gespiegelt.
// RTC_NOINIT_ATTR übersteht ESP.restart(), Reset-Taster und Neuflashen (kein
// Power-On) – siehe rtcRetryMagic weiter unten für dasselbe Muster. Vorher
// lag lastA1Day/lastA2Day nur im normalen RAM und wurde bei JEDEM Neustart
// auf 0xFFFF zurückgesetzt, auch bei einem harmlosen Reset kurz nach einem
// bereits erfolgreich gespielten Alarm. Lag dieser Neustart innerhalb des
// Nachholfensters (ALARM_CATCHUP_MIN), hielt alarmDue() den Alarm für
// verpasst und löste ihn erneut aus – nacheinander für Alarm 1 und Alarm 2,
// falls beide im Fenster lagen (beobachtet: beide Alarme sofort nach einem
// Reset ausgelöst, obwohl die aktuelle Uhrzeit nicht der Alarmzeit entsprach,
// sondern nur bis zu 60 Minuten danach lag). rtcDayGuardMagic unterscheidet
// echten Power-On (RTC-Inhalt undefiniert, Nachholfenster bleibt aktiv) von
// Reset/Neuflashen (RTC-Inhalt gültig, Tages-Sperre bleibt erhalten).
#define RTC_DAYGUARD_MAGIC 0xD4A6D4A6UL
RTC_NOINIT_ATTR uint32_t rtcDayGuardMagic;
RTC_NOINIT_ATTR uint16_t rtcLastA1Day;
RTC_NOINIT_ATTR uint16_t rtcLastA2Day;
...
static void lockAlarmDayGuard(uint8_t alarmNum) {
  ...
  if (alarmNum == 1) { lastA1Day = today; rtcLastA1Day = today; }
  else               { lastA2Day = today; rtcLastA2Day = today; }
}
...
void setup() {
  ...
  // ── Alarm-Tages-Sperre aus RTC-Speicher wiederherstellen ──
  // 20v16 (Bugfix): siehe rtcDayGuardMagic weiter oben. Nur bei gültigem
  // Magic-Wert (Reset/Neuflashen) wird die Sperre übernommen – bei echtem
  // Power-On (Magic ungültig) bleiben lastA1Day/lastA2Day auf ihrem
  // Compile-Default 0xFFFF, das Nachholfenster funktioniert wie bisher.
  if (rtcDayGuardMagic == RTC_DAYGUARD_MAGIC) {
    lastA1Day = rtcLastA1Day;
    lastA2Day = rtcLastA2Day;
  } else {
    rtcDayGuardMagic = RTC_DAYGUARD_MAGIC;
    rtcLastA1Day = lastA1Day;
    rtcLastA2Day = lastA2Day;
  }
  ...
  const esp_task_wdt_config_t twdt_cfg = {
    .timeout_ms    = WDT_HARDWARE_MS,  // aus SysConf_20v16.h
