...
// ── Konfiguration ────────────────────────────────────────────
#include "SysConf_12v09.h"                                                               // Pin-Belegung, Timing-Konstanten, Touch-Schwellwerte
#include "WEB.h"
...
static int16_t readStateDrained() {
  ...
  while (Serial2.available() >= DFPLAYER_RECEIVED_LENGTH) {
...
static void triggerAlarm(uint8_t alarmNum, uint8_t fileNo, uint8_t min, uint8_t failCount) {
  ...
  if (xSemaphoreTake(playerMutex, pdMS_TO_TICKS(200)) == pdTRUE) {
    ...
    while (Serial2.available()) { Serial2.read(); }                                        // Puffer verwerfen: verifyPlayStarted() soll garantiert nur die Antwort auf diesen playFolder sehen
...
void setup() {
  ...
  const esp_task_wdt_config_t twdt_cfg = {
    .timeout_ms    = WDT_HARDWARE_MS,  // aus SysConf_12v09.h
