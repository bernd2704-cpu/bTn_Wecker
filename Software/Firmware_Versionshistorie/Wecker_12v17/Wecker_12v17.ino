...
// ── Konfiguration ────────────────────────────────────────────
#include "SysConf_12v17.h"                                                               // Pin-Belegung, Timing-Konstanten, Touch-Schwellwerte
#include "WEB.h"
...
// webLogf() nur bei einer Änderung der Restbyte-Anzahl schreibt – bleibt der
// Wert über mehrere Abfragen hinweg gleich (z.B. dauerhaftes UART-Rauschen),
// würde sonst dieselbe Zeile beliebig oft im Web-Log erscheinen.
static uint32_t serial2LeftoverCount      = 0;
static int      serial2LeftoverLastLogged = 0;
...
static void checkSerial2Leftover(const char* label) {
  ...
  if (avail > 0) {
    ...
    serial2LeftoverLastLogged = 0;                                                       // Puffer leer – nächster Restbyte-Fund gilt wieder als neue Änderung
  }
}
...
void setup() {
  ...
  const esp_task_wdt_config_t twdt_cfg = {
    .timeout_ms    = WDT_HARDWARE_MS,  // aus SysConf_12v17.h
