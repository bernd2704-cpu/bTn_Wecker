...
// ── Konfiguration ────────────────────────────────────────────
#include "SysConf_20v02.h"                                                               // Pin-Belegung, Timing-Konstanten, Touch-Schwellwerte
#include "WEB.h"
...
static bool verifyPlayStarted(const char* label, uint8_t fileNo) {
  ...
  for (uint8_t attempt = 1; attempt <= VERIFY_PLAY_RETRIES; attempt++) {
    ...
    if (st > 0 || dfPlayerBusy()) { return true; }
  ...
  switch (alarmState) {
    ...
    case ALARM_RUNNING:
      if (delayFunction(t_start6, ALARM_POLL_MS)) {
        ...
        // Wiedergabe läuft (LOW) oder nicht (HIGH).
        bool mp3Finished = (playerStatus == 0)
                         || (playerStatus == -1 && !dfPlayerBusy());
        if (mp3Finished) {                                                               // MP3 beendet
...
void setup() {
  ...
  const esp_task_wdt_config_t twdt_cfg = {
    .timeout_ms    = WDT_HARDWARE_MS,  // aus SysConf_20v02.h
