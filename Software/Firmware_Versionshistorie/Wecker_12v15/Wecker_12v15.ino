...
// ── Konfiguration ────────────────────────────────────────────
#include "SysConf_12v15.h"                                                               // Pin-Belegung, Timing-Konstanten, Touch-Schwellwerte
#include "WEB.h"
...
void setup() {
  ...
  // Die zuvor angenommene automatische Boot-Initialisierung auf Core 3.x
  // trifft hier nicht zu (oder ist zeitlich nicht garantiert vor dem ersten
  // esp_task_wdt_add() der neu erzeugten Tasks). esp_task_wdt_init() zuerst
  // versuchen; ist das TWDT doch schon initialisiert, liefert es
  // ESP_ERR_INVALID_STATE – dann nur umkonfigurieren.
  ...
  const esp_task_wdt_config_t twdt_cfg = {
    .timeout_ms    = WDT_HARDWARE_MS,  // aus SysConf_12v15.h
  ...
  if (esp_task_wdt_init(&twdt_cfg) == ESP_ERR_INVALID_STATE) {
    esp_task_wdt_reconfigure(&twdt_cfg);                                                 // bereits initialisiert – nur umkonfigurieren
  }
